...
class SanitizeTabMixin:
    ...
    def start_scan_mapping(self) -> None:
        if not self._validate_scan_inputs():
            return
        self._start_worker("sanitize", self.sanitize_status_var, "[INFO] 开始识别候选映射...", self._scan_mapping_worker)

    def rescan_mapping(self) -> None:
        if not self.current_mapping_data:
            self.start_scan_mapping()
            return
        self._start_worker("sanitize", self.sanitize_status_var, "[INFO] 按当前映射继续识别候选映射...", self._scan_mapping_worker)
    ...
    def _scan_mapping_worker(self) -> None:
        ...
        payload = scan_file(
            ...
            existing_payload=self.current_mapping_data,
        )
        self.current_mapping_data = payload
        self.scan_ready = True
        self.root.after(0, self._after_scan_complete)

    def _after_scan_complete(self) -> None:
        self._rebuild_mapping_metadata()
        self._refresh_mapping_tree()
        self.sanitize_status_var.set("待确认")
        self.mapping_summary_var.set(self._mapping_summary_text())
    ...
    def apply_current_mapping(self) -> None:
        ...
        if not self.current_mapping_data:
            ...
            return
        self._start_worker("sanitize", self.sanitize_status_var, "[INFO] 开始生成脱敏文档...", self._apply_mapping_worker)

    def _apply_mapping_worker(self) -> None:
        self._rebuild_mapping_metadata()
        input_path = Path(self.sanitize_input_var.get().strip())
