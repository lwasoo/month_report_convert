...
class ConvertTabMixin:
    def _build_convert_tab(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(layout_group, text="formal 适合正式汇报；diversity=none 时不套用多样化版式。", style="Hint.TLabel").grid(row=3, column=0, columnspan=2, sticky="w", pady=(8, 0))
        ...
        ttk.Button(action_row, text="停止任务", command=self.stop_task).pack(side="left", padx=10)
    ...
    def start_convert(self) -> None:
        ...
        args = [
            ...
            "--ollama-url", self.ollama_url_var.get().strip() or DEFAULT_OLLAMA_URL,
            "--timeout", self.timeout_var.get().strip() or "180",
            "--retries", self.retries_var.get().strip() or "2",
            "--layout-mode", self.layout_mode_var.get().strip() or "formal",
            "--theme", self.theme_var.get().strip() or "formal_blue",
            "--diversity", self.diversity_var.get().strip() or "none",
